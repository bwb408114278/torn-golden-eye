...
final class OcTimelineEventScheduler {
     ...
     */
    record SimulationResult(boolean feasible, boolean deterministicFailure,
                            boolean hardObligationFailed, boolean plannedEmptyExpired,
                            List<OcLiquidityAnchor> anchors, List<OcPauseAssessment> pauses,
     ...
     */
    private record ObligationOutcome(boolean scheduled, boolean deterministicFailure,
        ...
        while (!queue.isEmpty()) {
            ...
            if (!outcome.scheduled()) {
                ...
                if (task.obligation().isHardObligation()) {
                    return new SimulationResult(false, deterministicFailure, true,
                            plannedEmptyExpired, state.anchors(), state.pauses(),
                            maxNewPause(state));
                }
            ...
            spawnSuccessors(queue, task, outcome.completionAt());
        }
        return new SimulationResult(true, deterministicFailure, false,
                plannedEmptyExpired, state.anchors(), state.pauses(), maxNewPause(state));
    }
     ...
     */
    private void spawnSuccessors(PriorityQueue<Task> queue, Task task,
                                 LocalDateTime completionAt) {
    ...
    private StageSchedule buildStageSchedule(OcTimelineState state,
        ...
        LocalDateTime firstJoinDeadline = obligation.firstJoinDeadline();
        ...
        for (OcPlannedAssignment assignment : assignments) {
            ...
            if (stageBoundary == null) {
                if (firstJoinDeadline != null && joinAt.isAfter(firstJoinDeadline)) {
                    return null;
                }
            } else if (joinAt.isAfter(stageBoundary)) {
                Duration pause = Duration.between(stageBoundary, joinAt);
                if (stageBoundary.isAfter(state.snapshotTime())
        ...
        return new StageSchedule(assignments, joinTimes, stageBoundary);
    }
     ...
     */
    private record Task(OcTimelineObligation obligation, List<OcTeamDemand> successors) {
    }
     ...
     */
    record CandidateRoot(OcTimelineObligation obligation, List<OcTeamDemand> successors) {
    }
     ...
     * @param completionAt 最终完成时间
     */
    private record StageSchedule(List<OcPlannedAssignment> assignments,
