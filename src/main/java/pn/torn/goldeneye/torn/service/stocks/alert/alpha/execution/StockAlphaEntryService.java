...
public class StockAlphaEntryService {
    ...
    private void validateExecutionBar(TornStockAlphaDecisionDO decision, LocalDate decisionDate, int phase,
        ...
        if (decision.getId() == null || decision.getDecisionBusinessDate() == null
                ...
                || !StockAlphaDecisionService.hasUsableSignalReferencePrice(decision)) {
     ...
     * 此时只标记不可复核并跳过本次入场,不写入批次、不扣减资金、不发送通知,
     * 也不把新结果冒充决策当时的排名事实。
     *
     * @param decision α决策
     */
    private void markSourceNotReproducible(TornStockAlphaDecisionDO decision) {
        log.error("α初始入场决策来源摘要不可复核,本次跳过入场: decisionId={}, decisionDate={}, phase={}, digest={}",
