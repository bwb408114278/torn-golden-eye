 ...
 * @since 2026.08.15
 */
public record OcRefreshSafetyResult(OcTimelineSafetyAssessment assessment,
    ...
    /**
     * 从全部已证明安全候选中提取非支配安全前沿向量。
     *
     * @return 按普通池、高阶池次数排序的非支配向量
     */
    public List<OcRefreshVector> frontier() {
        return candidates.stream().map(SafeCandidate::vector)
                .filter(vector -> candidates.stream().noneMatch(other ->
                        !other.vector().equals(vector)
                                && other.vector().normalCount() >= vector.normalCount()
                                && other.vector().highCount() >= vector.highCount()))
     ...
     */
    public record SafeCandidate(OcRefreshVector vector, PauseTier pauseTier,
                                BigDecimal windowValue, int incrementalMemberDays,
                                LocalDateTime earliestCompletionAt, int anchorCount,
