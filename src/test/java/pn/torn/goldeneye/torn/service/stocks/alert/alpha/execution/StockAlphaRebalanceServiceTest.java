...
class StockAlphaRebalanceServiceTest {
    ...
    @Mock
    private StockAlphaNoticeAuditWriter noticeWriter;
    @Mock
    ...
    void rebalance_persistsAndBindsDatabaseReplacementId() {
        ...
        when(decisionDAO.selectByExecutionKeyForUpdate(TRACK.trackCode(), LocalDate.of(2026, 9, 5), 0, EXECUTION_TIME))
                .thenReturn(decision);
        ...
        when(batchDAO.selectActiveAlphaBatchesForUpdate(StockPortfolioService.VIP_ALPHA_PORTFOLIO_CODE)).thenReturn(List.of(current));
        ...
        verify(noticeWriter).writeFormalRebalanceAudits(replacementCaptor.capture(), any(), any(),
                associationCaptor.capture());
        assertRebalanceNoticeAudit(replacementCaptor.getValue(), associationCaptor.getValue());
    }
    ...
    void rebalance_rejectsExistingReplacementBatchNo() {
        ...
        assertThrows(IllegalStateException.class, () -> service.rebalance(TRACK,
    ...
    void rebalance_alreadyExecuted_retryWritesNoDuplicateNoticeOrFact() {
        ...
        verify(noticeWriter, never()).writeFormalRebalanceAudits(any(), any(), any(), any());
    ...
    void rebalance_sourceNotReproducible_writesNoFact() {
        ...
        StockAlphaRebalanceService service = new StockAlphaRebalanceService(decisionDAO, slotDAO, batchDAO,
                portfolioService, noticeWriter, decisionService, new StockAlphaSlotPolicy(slotDAO));
    ...
    void rebalance_otherExecutionBar_writesNoFact() {
        ...
        when(decisionDAO.selectByExecutionKeyForUpdate(TRACK.trackCode(), LocalDate.of(2026, 9, 5), 0, laterExecutionBar))
                .thenReturn(null);
        ...
        verify(decisionDAO).selectByExecutionKeyForUpdate(TRACK.trackCode(), LocalDate.of(2026, 9, 5), 0, laterExecutionBar);
        verifyNoInteractions(slotDAO, batchDAO);
        verify(noticeWriter, never()).writeFormalRebalanceAudits(any(), any(), any(), any());
    ...
    void rebalance_executionBarNotStrictNext_rejectsWithoutAnySideEffect() {
        ...
        when(decisionDAO.selectByExecutionKeyForUpdate(TRACK.trackCode(), LocalDate.of(2026, 9, 5), 0, gapExecutionBar))
                .thenReturn(decision);
        when(decisionService.isSourceReproducible(decision)).thenReturn(true);
        when(slotDAO.selectAllByPortfolioCodeForUpdate(StockPortfolioService.VIP_ALPHA_PORTFOLIO_CODE))
                .thenReturn(List.of(slot()));
        when(batchDAO.selectActiveAlphaBatchesForUpdate(StockPortfolioService.VIP_ALPHA_PORTFOLIO_CODE)).thenReturn(List.of(currentBatch()));
        ...
        assertThrows(IllegalStateException.class,
                () -> service.rebalance(TRACK, DECISION_DATE, 0, processingTime, snapshot),
     ...
     *
     * @param noticeBatch 通知审计写入的换仓新仓批次
     * @param association 写入的换仓统一关联事实
     */
    private void assertRebalanceNoticeAudit(TornStockVirtualBatchDO noticeBatch,
