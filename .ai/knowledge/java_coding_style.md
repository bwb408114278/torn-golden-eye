...
- 复杂逻辑必须拆分成清晰的小方法。
- 所有外部输入必须校验。
- 本次修改涉及的关键业务操作必须有与风险匹配的测试，或明确说明现有测试已覆盖、无法测试或未执行的原因。
- 相同业务应当放在同一个包下，同个包下的文件或包过多时新建一个包并移入相应的包和文件
- 测试方法包含数据库操作时，必须使用@Rollback回滚测试产生的数据
...
测试用于保护本次修改的业务行为和高风险契约，不以测试数量、覆盖率或理论完整性作为扩大开发范围的理由。

测试范围必须与任务风险匹配：

- L1轻量修改：编译或静态检查、直接相关现有测试；仅在现有测试不足时新增一个主路径或直接回归测试；
- L2普通业务修改：核心正常流程、一个影响最大的失败流程、本次明确改变的边界；SQL变更增加真实Mapper或等价数据库行为验证；
- L3高风险修改：根据实际变更选择金额守恒、权限、事务回滚、并发幂等、迁移、故障注入或上线验收。

不得对L1/L2任务默认套用L3测试矩阵。即使是L3，也只覆盖本次修改相关的高风险契约，不穷举所有理论组合。
...
测试不要求机械覆盖以下全部类别。L1现有测试已经充分保护本次行为时可以不新增测试；L2/L3根据实际风险从以下类型中选择，不要求三类全部覆盖：
...
- 权限逻辑变更：权限校验测试；
- 状态机变更：合法和非法状态流转测试；
- 金额、收益或核心计算变更：关键计算边界和守恒测试；
- 事务写入变更：成功提交和一个有代表性的回滚测试；
- 并发或幂等逻辑变更：并发或重复执行测试；
- Mapper SQL或Schema变更：真实数据库行为或完整迁移测试；
- 第三方接口契约变更：一个成功契约和一个主要失败契约测试。

不要求为未修改的通用能力补齐历史测试。

## 一条规则，一个主证据

- 同一业务规则只选择一个主要测试层级完整验证，其他层只验证调用接线、参数传递或集成结果。
- 纯计算和判定规则优先使用快速单元测试。
- Service编排只验证关键调用、事务边界和结果传递，不重复纯函数的全部边界。
- Mapper SQL使用真实Mapper和数据库验证，禁止在测试中复制生产SQL后验证副本。
- 端到端测试只保留跨层后才能证明的核心路径，不重复所有单元和Mapper场景。
- 现有测试已经充分保护本次行为时，不要求为了新增代码机械新增重复测试。

## Review与回归范围

- P0安全漏洞、不可逆数据损坏、资金或关键数据严重错误、生产不可用必须阻断；P1本次主路径错误或有证据的严重回归默认阻断。
- P2低概率边界、代码整洁和证据增强默认不阻断；P3未来治理只记录建议。
- 多轮修复只重点回归未闭环P0/P1、本轮直接影响路径和少量核心门禁，不要求每轮重复执行全部历史测试矩阵。
- 全量测试仅在L3、影响面广、发布门禁要求或用户明确要求时默认执行；其他任务优先聚焦测试，并如实说明未执行范围。
...
- 禁止测试私有实现细节、框架自身行为或生产入口不可达的理论状态。
- 禁止用读取Java源码、XML或YAML并断言字符串存在，替代真实业务行为、事务、SQL或迁移验证。
- 禁止为了提高覆盖率复制同一业务规则的测试矩阵。
- 禁止因顺手发现的P2/P3问题扩大当前功能实现和测试范围。

## 测试停止条件

满足以下条件后，应停止继续增加测试：

1. 用户确认的验收标准已被测试或其他有效证据保护；
2. 编译或静态检查通过；
3. 与风险等级匹配的聚焦测试通过；
4. 没有未解决的P0/P1；
5. 没有证据表明本次修改引入数据、安全或可用性风险。

剩余低概率边界、未来治理和额外质量增强应记录为后续建议，不得自动阻断交付。
...
## `@version` 必须与项目最新版本号一致

- 修改类头或测试头 `@version` 前，必须先读取 `pom.xml` 的 `<version>` 得到当前项目最新版本号；
- 类/测试 `@version` 必须与当前项目版本号一致，禁止自行猜测或随意递增版本；
- 若确实需要提升版本，必须同步更新 `pom.xml` 与所有受影响文件的 `@version`，不得只改一处。

## 方法认知复杂度与规模门禁

- 方法 Cognitive Complexity 不得超过 SonarQube 门禁（≤ 15）；
- 方法参数不得超过7个

## 超级类治理

- 单文件过长（如超过 800 行）的类应在结构上拆分，避免超级类；
- 拆分属于大规模重构，必须经用户确认后再执行；未确认前记录为后续治理建议，不自动扩大当前任务范围。
