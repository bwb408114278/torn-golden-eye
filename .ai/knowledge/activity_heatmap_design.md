# 活跃度热力图设计方案（V2 历史基线，已被 V3 覆盖）

> **状态说明（2026.08.28）**：本文件只保留 V2 的历史背景与已实施能力说明，不再作为开发、测试或验收契约。
> 新需求（日期范围、配置帮派并集、Idle 按占比连续暗化、日终归档、帮派五档颜色、旧数据兼容）必须以
> [活跃度热力图 V3 技术实施方案](activity_heatmap_v3_technical_design.md) 为唯一依据；两份文档冲突时以 V3 为准。
