...
| 帮派颜色 | 按在线比例使用个人图的 0～100% 连续 Viridis 色板 | 仅帮派单图改为平均有效活跃人数的 5 档固定强对比色；对比图不变 |
...
边界按 `floor(averageActiveCount / 25)` 计算并 clamp 到 `[0, 4]`；不得按帮派成员数、在线比例、P95、当前图片、查询区间或用户配置自适应调整。

#### 3.3.2 固定 RGB 主色与按 Idle 占比连续暗化

5 个主色沿用可读性已验证的 Viridis 强对比锚点：

| 图例标签 | 主色 RGB | Idle 100% 时的最大暗化色 RGB |
|---|---|---|
| `0` | `(68, 1, 84)` | `(37, 1, 46)` |
...
- `idleRatio = 0`：使用完整主色；
- `idleRatio = 0.5`：使用主色与最大暗化色之间的中间亮度；
- `idleRatio = 1`：使用表中的最大暗化色（主色通道 × `0.55`，四舍五入）；
- 不使用 `I / memberCount`：暗化表达的是有在线证据群体中 Idle 对有效活跃的占比，而不是全体成员中的 Idle 占比；
- 颜色档位始终只由 `A` 决定，`I` 不改变格内数字、人数档位或图例标签；
- 主色和连续暗化后的颜色都继续通过既有亮度阈值选择黑白文字。

无数据始终为既有深灰 `(45,45,45)`，显示 `—`；已观测且有效活跃为 0 时，`I = 0` 使用档位 `0` 主色，`I > 0` 按公式连续暗化，必须与无数据区分。
...
| 帮派图 | 平均有效活跃人数 | `averageIdleCount / (averageActiveCount + averageIdleCount)` 按本节公式连续暗化，不改数字 | 本节固定 5 档人数色板 |
...
| P1 | `Idle` 仍进入 V3 active 主数字；V2/V3 同日被重复累计；归档失败仍写完成 marker；配置低段位帮派不能稳定纳入采集；日期边界/未来日期错误；无数据时伪造 0；帮派颜色未按冻结 5 档或 `I/(A+I)` 连续暗化规则实现 |
