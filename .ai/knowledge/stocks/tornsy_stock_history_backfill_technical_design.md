...
6. 回填入口只有两个：**超管机器人指令**人工指定任意历史范围 `[start,end)` 执行一次补数；**每日巡检**（07:00，Asia/Shanghai）检查昨天自然日的分钟连续性，发现缺口才自动补数。不使用 `sys_setting` 开关、启动自动恢复、每小时 AUTO 或 13 个月 EXPERIMENT 环境变量。
...
每天 **07:00（Asia/Shanghai）** 执行一次（`cron = 0 0 7 * * ?`）：
...
选择 07:00（2026-09-18 由 08:45 提前）：为 08:00 起的 α 决策窗口留出修复完成余量——α 的"已结束自然日快照构建 + 新决策"固定在 08:00 桶（08:15:10 处理），本巡检必须更早完成；昨天窗口在 00:00 后即稳定结束（实测最后一条分钟事实写入于当日 23:59），提前不影响判据、不与其他定时任务冲突，也不与实时采集的每分钟第 5 秒竞争。实测触发后 ≤2 分钟完成（08:45 档实测 08:45:39–08:46:31，改写 2–3 个桶共 70–105 根 bar）。巡检失败（failedSlices、HTTP 失败、重建异常）只记录 ERROR，不永久关闭；下一日仍会再次巡检。每日巡检只兜底近期缺口，不替代人工范围指令，也不自动回填数月/数年历史。
...
5. 次日 07:00 观察每日巡检日志：昨天全连续时不请求 Tornsy
