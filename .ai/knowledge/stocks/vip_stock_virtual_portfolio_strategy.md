...
- 一次性验收标准：`vip_stock_alert_business_acceptance_one_time.md`
- 相关文档：
...
5槽限制只属于正式组合。每槽`2.00b`是技术方案审核阶段冻结的生产资金口径，采用整数股数、余款现金和槽内复利；现有历史研究结果基于每槽`0.40b`，因此实施前必须按`2.00b`口径重新执行精确资金账本回放。6槽结果仅保留为容量敏感性证据，不再作为主业务参数。
...
| 动态自然SELL | `RESEARCH_DATA_ONLY / FORMULA_NOT_FROZEN` |
| HIGH风险硬否决 | `RESEARCH` |
...
### 13.3.1 日报与年度播报金额展示口径

日报和年度播报的金额展示统一使用十亿（`b`）作为单位，禁止使用“亿”作为主要展示格式，也禁止直接展示数据库金额的长小数：

```text
显示金额b = 原始金额 / 1,000,000,000
显示格式 = X.XXb
```
...
- 金额固定保留两位小数，采用四舍五入；
- 正收益金额显示`+X.XXb`，负收益金额显示`-X.XXb`，零值显示`0.00b`；
- 股票价格按当前数据实际精度展示两位小数；
- 股数显示整数；百分比与金额使用不同格式；
- 缺失或不可计算的权益显示“暂无法计算（行情数据不足）”，不得用买入成本、陈旧价或0代替；
- 正式组合、候选影子、无限资金影子、拒绝观察和槽位外机会的金额必须分区展示，不得混算。

年度结算以`Asia/Shanghai`每年1月1日00:00的边界权益为准。年度收益金额按期末权益减期初权益计算，年度展示基准只重置统计口径，不重置真实槽位现金、持仓、批次或历史账本。年度播报和日报必须复用同一金额展示规则；本节只冻结业务口径，具体格式化组件和年度播报入口由技术实现与一次性验收文档负责。

### 13.3.2 年度结算业务边界

- 每年1月1日结算上一自然年度，跨年持仓不强制SELL；
- 跨年持仓按零点边界权益归属前后年度，批次和成本保持不变；
- 期初/期末权益必须由完整合法行情计算，缺少边界行情时本次结算不得伪造完成；
- 年度事实与下一年度展示基准必须幂等、原子保存，服务重启可补偿但不得重复结算；
- 首个不完整年度同时展示实际区间收益率和年化折算收益率，并明确其不是收益承诺；
- 年度正式收益只统计`FORMAL`，槽位外机会单独统计，不进入正式组合年度权益、收益率或槽位利用率。

### 13.3.3 日报业务内容边界

日报可展示正式组合、候选影子、影子研究和槽位外机会，但必须明确标签和金额边界。动态SELL仍为`RESEARCH_DATA_ONLY`时，只展示研究输入状态，不展示为动态SELL建议数或正式收益。
