...
> **基线修订（2026-09-14，适用版本1.6.2起）**：本文第5章（大锅饭名单与常量设计）、第7.2节（帮派级扫描起点的固定日期分支）、第9.2节（排除规则数据结构的常量来源）中"以 `TornConstants` 常量承载名单与排除规则、`resolveIncomeStartTime` 内置PN/NOV固定起点"的基线设计，已升级为数据库表驱动方案，现行唯一基线见 `.ai/knowledge/oc/oc_reassign_db_driven_technical_design.md`。本文保留为历史实施记录与业务不变量来源；其中日期边界语义（左闭区间、生效前普通收益保留）、"普通计入次数+大锅饭计入次数=1"、不删除/不迁移飞书普通收益、不加写侧过滤等决策在新方案中继续有效。
...
## 5. 大锅饭名单与常量设计（已升级：名单与规则自1.6.2起改为表驱动，见基线修订）
...
### 7.2 扫描起点（已升级：自1.6.2起改为按范围行最小生效时间派生，见基线修订）
