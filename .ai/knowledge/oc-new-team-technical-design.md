...
对含主动新增暂停的收益候选，逐随机组合先构建可行的零新增暂停基准；只有该候选按同一分层比较**严格优于**基准，才允许因暂停增加刷新建议。基准不可行、不可比较或证据不足时，不能以暂停提高建议；零暂停候选不受该“优于基准”前置条件限制。均衡级主动新增停转同样复用该全局零停转基准：它只适用共同硬门禁与“价值提高或完整释放提前”的OR准入，不适用收益级单位成员人天严格改善门禁。
...
| 均衡 | 满足6小时政策及相对全局零停转基准的价值/完整释放准入 → 更大的已证明联合向量 → 更少待启动期限压力/更连续释放 → 普通高阶平衡 → 稳定tie-break。 |
| 收益 | 满足12小时政策 → 规划窗口全局总价值 → 增量单位成员人天 → 更早释放稀缺岗位 → 刷新总数 → 稳定tie-break。 |

已证明至少存在一个安全刷新向量时，不得因旧百分比取整返回0。均衡模式的停转候选准入过滤先于排序发生：未通过相对零停转基准价值/完整释放准入的`WITHIN_BALANCED`候选不参与均衡排序，排序本身不再为其提供胜出机会。
...
阶段二：先形成全部已证明安全的 ZERO_PAUSE 候选集合，再选择全局基准，
        并使用同一基准分别重新判定 WITHIN_BALANCED 与 WITHIN_PROFIT 候选。
```
...
阶段二仅在零停转候选集合可公平比较时执行。对于 WITHIN_BALANCED，除共同硬门禁不退化外，必须相对该基准满足“业务价值严格提高 OR 保证完整释放严格提前”；不适用收益级单位成员人天严格提高门禁。对于 WITHIN_PROFIT，继续适用本节完整收益严格改进门槛。搜索超时、组合预算耗尽、匹配替代上限命中、搜索边界无法证明完整、基准缺失、证据不足、既有义务延迟不可证明或保证释放不可比较时：

```text
保留已证明安全候选、lowerBound和匿名搜索遥测；
所有 WITHIN_BALANCED 与 WITHIN_PROFIT 候选 fail-closed；
不得因主动新增停转提高刷新建议。
```

实现边界：`search.OcRefreshVectorSearcher`负责阶段一编排、完整性判断和阶段二候选收敛；阶段二在统一形成零停转基准后，分别收敛`WITHIN_BALANCED`与`WITHIN_PROFIT`的模式资格。`search.OcRefreshVectorEvaluator`只生成向量与随机组合的安全/价值事实或提供最终重判协作；`evidence.OcEconomicValueComparator`继续提供固定价值排序和严格比较。不得为了形成基准重新模拟同一向量，也不得以扩大预算或改变快照规避fail-closed。

`avoidableExpiryPressure`表示“存在计划内无人OC可避免过期压力”，必须在时间线摘要中保持该正向语义。多随机组合聚合时，任一组合出现该压力，向量摘要必须保留该压力；不得取反，也不得用逻辑与使其他安全组合掩盖风险。

Shadow中的“正向候选已证明”只表示候选同时具备完整可比较价值证据和对应模式资格，不得仅依据向量数量、停转层级或阶段一安全状态置位。Shadow、阶段二搜索和模式选择必须复用同一全局零停转基准事实；日志层不得重新构造比较器或重新解释候选资格。Shadow新增的匿名均衡资格字段（如`selectedBalancedPauseCandidateEligible`）仅记录已选候选的阶段二均衡准入结论，不重算比较，也不得借此输出成员、岗位或奖励明细。
